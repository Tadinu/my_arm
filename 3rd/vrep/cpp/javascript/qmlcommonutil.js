...
.import "qrc:///javascript/qmlcommonresource.js" as RBRC // This file is the samefor all apps
...
var UTIL = {
    ...
    inchToMM: function(value, dimension) {
        if(dimension === undefined)
            dimension = 1;
        return Math.pow(RBRC.VAL.INCHTOMM, dimension) * value;
    },

    mmToInch: function(value, dimension) {
        if(dimension === undefined)
            dimension = 1;
        return Math.pow(RBRC.VAL.MMTOINCH, dimension) * value;
    },
...
var TEXT = {
    ...
    getContextMenuItemText : function (itemNamePath) {
        ...
        return (baseName.length > RBRC.TEXT.ITEM_MAX_LEN)?
                baseName.substr(0, RBRC.TEXT.ITEM_MAX_LEN) + "..." + baseName.slice(extItemText.lastIndexOf('.'), baseName.length); // ".STL"
                baseName;
        */
        /*
        fileName = ‘file.jpeg’;
    ...
    getContextMenuItemShrinkedText : function (itemText) {
        if (itemText.length > RBRC.TEXT.ITEM_MAX_LEN) {

            itemText   = itemText.substr(0, RBRC.TEXT.ITEM_MAX_LEN) + "...";
        }
        return itemText;
    },

    cutTextItemLastElement : function (itemText, maxLength){
        if(RBRC.CONST_STRING.indexOf(itemText) !== -1)
            return itemText;

        if(maxLength === undefined)
            maxLength = RBRC.SIZE.ITEM_TEXT_MAX_LENGTH + 3;
        var itemName;
        var itemFormat;
        ...
        if (RBRC.PROJ_OBJ_FORMAT.indexOf(itemLastElements) !== -1){
    ...
    cutTextItemLastElementNoDot : function (itemText, maxLength) {
        if(maxLength === undefined)
            maxLength = RBRC.SIZE.ITEM_TEXT_MAX_LENGTH;
    ...
    getContextMenuItemShrinkedTextII : function (itemText, maxlength) {
        if (itemText.length > maxlength) {
            itemText   = "..." + itemText.substr(itemText.length - RBRC.TEXT.ITEM_MAX_LEN, RBRC.TEXT.ITEM_MAX_LEN);
        }
