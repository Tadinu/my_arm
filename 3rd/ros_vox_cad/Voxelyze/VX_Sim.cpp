...
//	for (int k=0; k<iT; k++) BondArrayCollision[k].ResetBond();
	...
	for (int i=0; i<NumVox(); i++) 
	{
		...
		if(pEnv->pObj->GetUsingInitialVoxelSize())
		{
			...
			VoxArray[i].setVoxelScale(VoxArray[i].initialVoxelSize);  // immediately set scale of voxels (pre-natal)
		}
		else
		{
		...
		{
		    VoxArray[i].startGrowthTime = pEnv->pObj->GetStartGrowthTime(i) * (GetStopConditionValue() - GetInitCmTime()) + GetInitCmTime();
		}
		else
		{
		    if(pEnv->pObj->GetUsingFinalVoxelSize() || pEnv->pObj->GetUsingGrowthTime())
		    {
                VoxArray[i].startGrowthTime  = GetInitCmTime();  // start growing asap
            }
            else
            {
                VoxArray[i].startGrowthTime  = GetStopConditionValue(); // never start growing
            }
		}
		...
		{
            double growTimeFromVxa = pEnv->pObj->GetGrowthTime(i) * (GetStopConditionValue() - VoxArray[i].startGrowthTime);
            VoxArray[i].growthTime = (growTimeFromVxa <= 0.05) ? 0.05 : growTimeFromVxa;

//            // complete genetic assimilation: THIS DOESN'T WORK (B/C CHANGING SCALE TOO FAST?)
//            if (growTimeFromVxa + VoxArray[i].startGrowthTime <= 0.05 + GetInitCmTime())
//            {
//                if (pEnv->pObj->GetUsingInitialVoxelSize() && pEnv->pObj->GetUsingFinalVoxelSize())
//                {
//                    VoxArray[i].initialVoxelSize = VoxArray[i].finalVoxelSize;
//                    VoxArray[i].setVoxelScale(VoxArray[i].finalVoxelSize);
//                }
//            }
		...
		else
		{
		    if(pEnv->pObj->GetUsingFinalVoxelSize())
		    {
		        ...
		        VoxArray[i].growthTime  = GetStopConditionValue() - VoxArray[i].startGrowthTime; // equivalent to stopCondition - initCmTime
                // VoxArray[i].growthTime  = 0.05;
		    }
            else
            {
                VoxArray[i].growthTime  = 0.05;  // this never happens because we need a final size to grow
            }
		}
        ...
        if(pEnv->pObj->GetUsingFinalVoxelSize())
        {
			...
			VoxArray[i].finalVoxelSize = finalVoxelSize;
			 ...
			 // if(pEnv->pObj->GetUsingGrowthTime() && VoxArray[i].startGrowthTime + VoxArray[i].growthTime <= 0.05)
			     // // (complete) genetic assimilation --> if time to final is close to zero then immediately set scale (before initCmTime)
			     // VoxArray[i].initialVoxelSize = finalVoxelSize;
			     // VoxArray[i].setVoxelScale(VoxArray[i].initialVoxelSize);
        }
        else
        {
...
double CVX_Sim::computePointingError(int step)
{
    ...
    //pointingError = acos((A.Normalized()).Dot(B.Normalized()))*180.0/VOXEL_PI;

    pointingError = acos(safeAcosArgument((A.Normalized()).Dot(B.Normalized())))*180.0/VOXEL_PI;
...
bool CVX_Sim::StopConditionMet(void) //have we met the stop condition yet?
{
	...
	vfloat fNumVoxInv;
	if(CurTime <= 2*InitCmTime) // Evaluate stop condition only after the transient time is expired
		return false;
...
//			if(pEnv->pObj->GetUsingFinalVoxelSize() || pEnv->pObj->GetUsingInitialVoxelSize())
//		 		return CurTime>(1.25*StopConditionValue) ? true:false; // Regime Comparision: keep going for another 0.25*StopConditionValue
//		 	else
