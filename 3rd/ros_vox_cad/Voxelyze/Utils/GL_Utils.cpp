...
unsigned char *CGL_Utils::_image = 0;
int CGL_Utils::_imgWidth  = 0;
int CGL_Utils::_imgHeight = 0;
#endif
...
#ifdef GL_DRAW_IMAGE_DUCTA
void CGL_Utils::SetImage(unsigned char* img, int dataSize)
{
    if(!_image){
        delete [] _image;
        _image = 0;
    ...
    _image = new unsigned char[dataSize];
    for(int i = 0; i < dataSize; i++) {
        _image[i] = img[i];
    }
}

void CGL_Utils::DrawImage()
{
    ...
    glRasterPos2f(-1, 1);
    glRasterPos2i(0, 0);
    glDrawPixels(_imgWidth, _imgHeight, GL_RGB, GL_UNSIGNED_BYTE, _image);
}

void CGL_Utils::SetImageSize(int width, int height)
{
    _imgWidth  = width;
    _imgHeight = height;
}
