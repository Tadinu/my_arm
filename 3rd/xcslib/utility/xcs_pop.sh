...
if test $# -ge 1
  then # specificati almeno due parametri: procede ...
    ...
    do
      ...
      if test "$interval" = 1
         then 
		   zcat -f $inFileName | gawk '{if ($NF=="Testing") print $5;}' | moving_average $window > $outFileName
		 else   
		   zcat -f $inFileName | grep Testing\$ | gawk '{print $5}' | moving_average $window | \
  	...
  	echo ""
	echo "USAGE: xcs_pop.sh [-w <window>] [-i <interval>] <input files>"
  	echo ""
