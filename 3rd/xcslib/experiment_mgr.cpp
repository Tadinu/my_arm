...
experiment_mgr::experiment_mgr(xcs_config_mgr2 &xcs_config)
{
...
#ifdef __DEBUG__
	sprintf(output, __EXPERIMENT_MGR_CFG_IN__, __EXPERIMENT_MGR_VARS_OUT__);
	clog << output << endl;
#endif
	...
	xcs_utility::set_flag(string(str_save_agent_state), flag_save_agent_state);
	xcs_utility::set_flag(string(str_save_agent_report), flag_save_agent_report);
	xcs_utility::set_flag(string(str_save_experiment_state), flag_save_state);
	xcs_utility::set_flag(string(str_trace_time), flag_trace_time);
	xcs_utility::set_flag(string(str_compact_mode), flag_compact_mode);
...
experiment_mgr::save_agent_report(const unsigned long expNo, const unsigned long problem_no) const
{
	...
	if (!output.good())
	{
		...
		xcs_utility::error(class_name(),"save_agent", msg,1);
	}
