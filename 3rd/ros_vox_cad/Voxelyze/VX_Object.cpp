...
// The function below reads the XML voxcad input file (.vxa) defining the simulation
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
    ...
    if (pXML->FindElement("GrowthTime"))
    {
        usingGrowthTime = true;
        int voxCounter = 0;

        InitGrowthTimeArray(X_Voxels*Y_Voxels*Z_Voxels);
        for (int i=0; i<Z_Voxels; i++)
        {
            std::string DataIn;
            std::string RawData;
            ...
            for (int k=0; k<X_Voxels*Y_Voxels; k++)
            {
                if (pData[X_Voxels*Y_Voxels*i+k] > 0)
                {
                    SetGrowthTime(voxCounter,atof(dataArray[k].c_str()));
                    voxCounter++;
                }
            }
        }
    ...
    {
        usingGrowthTime = false;
    }



    if (pXML->FindElement("StartGrowthTime"))
    {
        usingStartGrowthTime = true;
        int voxCounter = 0;

        InitStartGrowthTimeArray(X_Voxels*Y_Voxels*Z_Voxels);
        for (int i=0; i<Z_Voxels; i++)
        {
            std::string DataIn;
            std::string RawData;
            ...
            for (int k=0; k<X_Voxels*Y_Voxels; k++)
            {
                if (pData[X_Voxels*Y_Voxels*i+k] > 0)
                {
                    SetStartGrowthTime(voxCounter,atof(dataArray[k].c_str()));
                    voxCounter++;
                }
            }
        }
	...
	{ 
		...
		InitStiffnessPlasticityRateArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetStiffnessPlasticityRate(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	{ 
		...
		InitKPArray(X_Voxels*Y_Voxels*Z_Voxels);
		...
		for (int i=0; i<Z_Voxels; i++)
		{
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetKP(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	{ 
		...
		InitKIArray(X_Voxels*Y_Voxels*Z_Voxels);
		...
		for (int i=0; i<Z_Voxels; i++)
		{
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetKI(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	{ 
		...
		InitANTIWINDUPArray(X_Voxels*Y_Voxels*Z_Voxels);
		...
		for (int i=0; i<Z_Voxels; i++)
		{
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetANTIWINDUP(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
