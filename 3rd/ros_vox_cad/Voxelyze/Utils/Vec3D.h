...
#define VOXEL_PI 3.14159265358979
...
//		if (theta < DISCARD_ANGLE_RAD) {*this = CQuat(1,0,0,0); return;} //very small angle, return no rotation
		...
		Axis.NormalizeFast();
        if (theta > VOXEL_PI-DISCARD_ANGLE_RAD) {*this = CQuat(Axis); return;} //180 degree rotation (180 degree rot about axis ax, ay, az is Quat(0,ax,ay,az))
...
//		vfloat theta = acos(RotateFrom.Dot(Vec3D(1,0,0))); //because RotFromNorm is normalized, 1,0,0 is normalized, and A.B = |A||B|cos(theta) = cos(theta)
        if (theta > VOXEL_PI-DISCARD_ANGLE_RAD) {w=0; x=0; y=1; z=0; return;} //180 degree rotation (about y axis, since the vector must be pointing in -x direction
