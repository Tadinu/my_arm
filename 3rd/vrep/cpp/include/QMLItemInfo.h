#ifndef QML_ITEM_INFO_H
#define QML_ITEM_INFO_H
...
class QMLItemInfo : public QObject
{
    Q_OBJECT
        ...
        //Q_PROPERTY(QQmlListProperty<QMLItemInfo> extendedList READ extendedList)
public:
    QMLItemInfo(QObject* parent = nullptr) : QObject(parent) {
        initialize();
    }

    QMLItemInfo(int itemId,
    ...
    QMLItemInfo(const QString& text, int opId,
        QObject* parent = nullptr) : QObject(parent) {
        initialize();
    ...
    QMLItemInfo(const QString& text, int itemId,
    ...
    /*
    // extendedList --
    QQmlListProperty<QMLItemInfo> extendedList() {
        return QQmlListProperty<QMLItemInfo>(this, _extendedList.toList());
        //return QQmlListProperty<QMLItemInfo>(this, 0, &QMLItemInfo::append_item);
    }
    */

    static void append_item(QQmlListProperty<QMLItemInfo> *list, QMLItemInfo *item)
    {
        QMLItemInfo *itemList = qobject_cast<QMLItemInfo *>(list->object);
        if (item)
...
private:
    ...
    QVector<QMLItemInfo *> _extendedList;
