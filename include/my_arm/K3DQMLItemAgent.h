#ifndef K3D_DIALOG_AGENT_H
#define K3D_DIALOG_AGENT_H

#include <assert.h>
#include <QObject>
#include <QKeyEvent>

#define K3D_DIALOG_PP(dialogP) reinterpret_cast<K3DQMLItemAgent**>(&dialogP)

class K3DQMLItemAgent : public QObject
{
    Q_OBJECT

public:
    explicit K3DQMLItemAgent(QObject *parent = 0, bool isBusy = false, 
                             int itemId = -1, QObject* itemUI = nullptr);
    virtual ~K3DQMLItemAgent();
...
private:
    /* IF SINGLETON:
        explicit K3DQMLItemAgent(QWidget *parent = 0);
        static K3DQMLItemAgent *_instance;
    */
    // Is being shown:
    bool _isActivated;
