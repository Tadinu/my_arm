...
QtObject {
    ...
    property real _SCREEN_WIDTH  : Screen.width > 0  ? Screen.width  : RBRC.SIZE.SCREEN_FULL_HD_WIDTH  // FULL HD // Actually, QtObject does not know about Screen -_- property real _SCREEN_HEIGHT : Screen.height > 0 ? Screen.height : RBRC.SIZE.SCREEN_FULL_HD_HEIGHT // FULL HD
    property real _SCREEN_HEIGHT : Screen.height > 0 ? Screen.height : RBRC.SIZE.SCREEN_FULL_HD_HEIGHT // FULL HD
    ...
    property int _BASE_FONT_SIZE: _SCREEN_HEIGHT <= RBRC.SIZE.SCREEN_MIN_HEIGHT ?
                                  RBRC.SIZE.TEXT_BASE_SIZE : (_SCREEN_HEIGHT * RBRC.SIZE.STANDARD_SCALE);
    ...
    function getRefSize(size) {
        return size * (_SCREEN_HEIGHT / RBRC.SIZE.SCREEN_FULL_HD_HEIGHT);
    }
    ...
    function convertSpinboxValidation() {
        ...
        var isSpinBox         = spinBox.objectName === RBRC.TEXT.K3D_SPINBOX_OBJECT_NAME;
        if(!isSpinBox && spinBox.objectName !== RBRC.TEXT.K3D_SPINBOX_ITEM_OBJECT_NAME)
            return;
        /*
        if(isSpinBox)
        ...
        */
        ...
        if (needValidateRange) {
            min = isCurrentlyMM ? min : min * Math.pow(RBRC.VAL.MMTOINCH, dimension);
            max = isCurrentlyMM ? max : max * Math.pow(RBRC.VAL.MMTOINCH, dimension);
        }
    ...
    function dimAllShownModelessDialogsBUT(dialogId) {
        ...
        for(var i = 0; i < _mainWindow.children.length; i++) {
            if ((RBUTIL.UTIL.qmltypeof(_mainWindow.children[i], "QQuickLoader")) &&
    ...
    function dimAllCurrentModelesses() {
        ...
        for(var i = 0; i < _mainWindow.children.length; i++) {
            if ((RBUTIL.UTIL.qmltypeof(_mainWindow.children[i], "QQuickLoader")) && // Dialog Loader [Modal or Modeless]
                (_mainWindow.children[i].item !== null) &&
                (_mainWindow.children[i].item._isBeingShown === true) &&
                (_mainWindow.children[i].item._isModal === false)
                ) {
    ...
    */
    ...
    //
    function tr(text) {
        return (_QML_ADAPTER !== null) ? text + _QML_ADAPTER.dummyTextAgent :
                                             text;
    }
