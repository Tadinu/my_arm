...
public:
	...
	bool writeQhullInputFile(); // dump current position of all voxels to QhullTmpFile, to be interpreted by qhull for convex hull calculation
	double invokeQhull(); // invoke qhull and return the volume of the convex hull for the robot state dumped to QhullTmpFile
	double computeAndStoreQHullStart(bool usingGUIin = false){ usingGUI = usingGUIin; updateDeformableMesh(); qhullVolumeStart = invokeQhull(); return qhullVolumeStart; }
	double computeAndStoreQHullEnd(bool usingGUIin = false){ usingGUI = usingGUIin; updateDeformableMesh(); qhullVolumeEnd = invokeQhull(); return qhullVolumeEnd; }
	double computeCurrentRobotVolume();
	double computeAndStoreRobotVolumeStart(){ robotVolumeStart = computeCurrentRobotVolume(); return robotVolumeStart; };
	double computeAndStoreRobotVolumeEnd(){  robotVolumeEnd = computeCurrentRobotVolume(); return robotVolumeEnd; };
	void initializeDeformableMesh(CVX_Sim* pSimIn);
	...
	double getRobotVolumeStart(){ return robotVolumeStart; }
	double getRobotVolumeEnd(){ return robotVolumeEnd; }

	double getRobotQhullVolumeStart(){ return qhullVolumeStart; }
	double getRobotQhullVolumeEnd(){ return qhullVolumeEnd; }

	// Shape complexity
	double computeInitialShapeComplexity();
	double computeFinalShapeComplexity();
	double computeShapeComplexity(std::string tmpFileName);	

	double getShapeComplexityStart(){ return shapeComplexityStart; }
	double getShapeComplexityEnd(){ return shapeComplexityEnd; }
	
private:
	int tipVoxel;
	double qhullVolumeStart;
	double qhullVolumeEnd;
	double robotVolumeStart;
	double robotVolumeEnd;

	double shapeComplexityStart;
	double shapeComplexityEnd;
